...
--given an ideal generated by pseudomonomials, produces the corresponding neural code
--eventually, given an ideal generated by squarefree monomials, will produce the corresponding neural code
neuralCode Ideal := NeuralCode => { Polarized => false } >> opts -> I -> (
	...
	if I==ideal(1_R) then neuralCode(allCodeWords(d)) else (
		...
		validCode := true;
		for j in L do (
		    M := matrix{flatten{apply(d,k->sub(value(i#k),R)),apply(d,k->sub(1-value(i#k),R))}};
		    if sub(j,M) != 0 then (validCode = false; break)
		    );
		if not validCode then continue else i
		);
	    depol := (ZZ/2)[first entries submatrix(vars R,{0..(d-1)})];
...
--	    validCode := true;
--	    for j in L do (
--		M := if opts.Polarized then (matrix{flatten{apply(d,k->sub(value(i#k),R)),apply(d,k->sub(1-value(i#k),R))}}) else matrix{apply(d,k->sub(value(i#k),R))};
--		if sub(j,M) != 0 then (validCode=false; break);
--		);
--	    if not validCode then continue else i
--	    );
--	depol:=if opts.Polarized then (ZZ/2)[first entries submatrix(vars R,{0..(d-1)})] else R;
--	if opts.Polarized then neuralCode(codeList,depol,R) else neuralCode(codeList,depol)
--	)
--    )
