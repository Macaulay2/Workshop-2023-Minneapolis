...
canonicalForm Ideal := List => { Factor => false, SharedIndex => false } >> opts -> I -> (
...
canonicalForm NeuralCode := List => { Factor => false, Iterative => true } >> opts -> C -> (
    if opts.Iterative then (
	if not C.cache.?canonicalForm then C.cache.canonicalForm = iterCanonicalForm(C);
	if opts.Factor then apply(C.cache.canonicalForm,factor) else
	C.cache.canonicalForm
	)
    else
    if not C.cache.?canonicalForm then C.cache.canonicalForm = canonicalForm(neuralIdeal(C));
...
polarizedCanonicalForm NeuralCode := List => C -> (
    S := polarizedRing C;
    if not C.cache.?polarizedCanonicalForm then C.cache.polarizedCanonicalForm = polarizeList(canonicalForm(C),S);
...
isCanonical Ideal := Boolean => opts -> I -> ( -- I -> I.cache.isCanonical ??= (
...
document{
    Key => {polarizedCanonicalIdeal,(polarizedCanonicalIdeal,NeuralCode),(polarizedCanonicalIdeal,NeuralCode),(polarizedCanonicalIdeal,Ideal,Ring)},
