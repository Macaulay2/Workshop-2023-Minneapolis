...
ZZdfactorization HashTable := ZZdFactorization => opts -> maps -> (
    ...
    if R.cache.?rootOfUnity then C.cache.rootOfUnity = R.rootOfUnity;
    C
    )
...
Fold(ComplexMap,ZZ) := (phi,d) -> (
    ...
    map(Fold(target phi,d),Fold(source phi,d),new HashTable from Lk,Degree => degree phi)
    )
...
Hom(Matrix, ZZdFactorizationMap) := ZZdFactorizationMap => (f,g) -> f**g
    ...
    maps := new HashTable from for i from lo to hi list i => (
       mats := for ba in indices DC_i list
           for ab in indices CD_i list (
              ...
              -- should be the zero map, unless a+b == i modulo the period
              if (a+b)%hi === i%hi then 
...
-*
----KELLER: UPDATE TO WORK FOR ARBITRARILY LARGE PERIOD
