...
bracketRing (VisibleList, ZZ) := o -> (vectorSymbols, d) -> (
    ...
    ret := new BracketRing from {numrows => n, numcols => d, ring => S, ideal => I, table => lookupTable, Strategy => o.Strategy, cache => new CacheTable from {}};
...
*-
  ...
  -- check monomial order
	    G := Grassmannian(d - 1, #vectorSymbols - 1);
      B := bracketRing(vectorSymbols, d);
      GB := forceGB matrix {apply(
        gens ring G, 
        v -> (
            S := last baseName v;
            A := [new Array from apply(S, i -> vectorSymbols#i)];
            (A_B)#RingElement
            )
        )};
...
Array _ AbstractGCRing := (A, R) -> (
    ...
    A1 := new Array from sort toList A0;
    ...
    rowSet := first select(1, values B#table, v -> y_A1 == v);
...
normalForm GCExpression := b -> (
    ...
    if B.Strategy === Grassmannian then (
      nf := r % B#cache#gb;
    )
    else nf := (r % I);
    nf_B
