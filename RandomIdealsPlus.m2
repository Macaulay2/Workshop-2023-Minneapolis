...
randomMultiNomialIdeal = method(Options => {Pure => false})
randomMultiNomialIdeal(ZZ,List,Ring) := o -> (N,Degs,S) -> (
...
multiMixedIdeal = method(Options => {Pure => false})
multiMixedIdeal(List,List,Ring) := o -> (TermList,DegList,S) -> (
    sum apply(#DegList, i -> randomMultiNomialIdeal(TermList#i,DegList#i,S,Pure => o.Pure))
    )
...
Key
  randomMultiNomialIdeal
  (randomMultiNomialIdeal, ZZ, List, Ring)
  [randomMultiNomialIdeal, Pure]
Headline
 Computes a random ideal generated by homogeneous elements with a fixed number of terms.
Usage
  I  = randomMultiNomialIdeal(N,Degs,S)   
Inputs
 N: ZZ
 Degs: List
 S: Ring
...
Description
  ...
  Example
    ...
    N = 4
    Degs = {4}
    I = randomMultiNomialIdeal(N,Degs,S, Pure => true)
///
...
Description
  ...
  Example
    ...
    TermList = {2,3}
    DegList = {{5},{3}}
    I = multiMixedIdeal(TermList,DegList,S,Pure => true)
///
...
setRandomSeed 0
assert(randomMultiNomialIdeal(3,{3},S) == ideal( -30*a^2*b + 24*b^2*c + 19*a*c*d))
setRandomSeed 0
assert(multiMixedIdeal({2,3},{{2},{2}},S) == ideal(-10*a*d - 8*c*d, 19*a*c + 39*b*c - 24*c*d))
R = ZZ/101[a,b,c]
D = {2,3,4}
E = {2,3}
setRandomSeed(0)
