...
isWellDefined SimplicialModule := Boolean => C -> (
    ...
    if C.?ss then (
	if not isSimplicial C then (
	    if debugLevel >0 then (
		<< "--object fails to satisfy simplicial identities; run isSimplicial to see where it fails" << endl;
		);
	    return false;
	    );
	);
...
isSimplicial = method()
isSimplicial(SimplicialModule) := Boolean => S -> (
