 ...
 load "SimplicialMapUtilities_Modified.m2"
...
simplicialModuleModified = method(Options => {Base=>0,Degeneracy => false})
simplicialModuleModified(Complex,HashTable,HashTable,ZZ) := SimplicialModule => opts -> (C,H1,H2,d) -> (
    spots := sort keys H1;
    if #spots === 0 then
...
--H1 is the face maps
simplicialModuleModified(Complex,HashTable,ZZ) := SimplicialModule => opts -> (C,H1,d) -> (--print("made it here!");
    spots := sort keys H1;
    if #spots === 0 then
      error "expected at least one map";
    R := ring C;
...
simplicialModuleModified(HashTable,HashTable,HashTable,ZZ) := SimplicialModule => opts -> (L,H1,H2,d) -> (--print("we got started");
    R := ring (L#((keys L)#0));
    S := new SimplicialModule from {
	symbol ring => R,
	symbol topDegree => d,
...
simplicialModuleModified(HashTable,HashTable,ZZ) := SimplicialModule => opts -> (L,H1,d) -> (--print("we got started");
    R := ring (L#((keys L)#0));
    S := new SimplicialModule from {
	symbol ring => R,
	symbol topDegree => d,
...
simplicialModuleModified(Complex,ZZ) := SimplicialModule => opts -> (C,d) -> (
    --C is a chain complex, output is the Dold-Kan image of C in the category of simplicial modules
	 ...
	 if opts.Degeneracy == true then (degenmapHash := hashTable flatten for n from 1 to d-1 list (
		...
		(n,i) => degenMapiModified(n,i,C)
		)
	     ););
	 facemapHash := hashTable flatten for n from 1 to d list (
	     for i from 0 to n list (
	 	 (n,i) => faceMapiModified(n,i,C)
	 	 )
	     );
 ...
 simplicialModuleModified(Complex) := SimplicialModule => opts -> C -> (simplicialModule(C,length C,Degeneracy => opts.Degeneracy))
 
 simplicialModuleModified(Module,ZZ) := SimplicialModule => opts -> (M,d) -> (simplicialModule(complex M,d,Degeneracy => opts.Degeneracy))
