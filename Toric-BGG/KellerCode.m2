...
differentialModule Ring := R -> differentialModule map(R^1,R^1,0);
...
resDM(DifferentialModule) := opts -> (D) ->(
    n := opts.LengthLimit;
    d := degree D;
    cyc := res(kernel D,LengthLimit =>n+1);
...
--any morphism of complexes should induce a morphism of the folds
--Input: Chain complex map
--Output: corresponding morphism of differential modules
mapDM = method();
mapDM(ChainComplexMap) := phi -> (low := min {min source phi, min target phi};
    high := max { max source phi, max target phi};
    L := new MutableList;
    for i from low to high do (L#(#L) = phi_i);
    directSum toList L
    )

--the induced map of differential modules coming from a 
--comparison map of complexes
--Inputs: Two chain complexes and a map in homological degree 0,
--Outputs: the induced comparison map, viewed as a morphism of differential modules.
extendDM = method();
extendDM(ChainComplex,ChainComplex,ModuleMap) := (F,G,phi) -> (mapDM(extend(F,G,phi)));


--tensor product of complex and differential module or 
--map of complexes and differential module
--Inputs: chain complex and differential module, or chain complex map and differential module
--Outputs: the tensor product of the chain complex (map) with the differential module.
tensorDM = method();
tensorDM(ChainComplex,DifferentialModule) := (C,D) -> (compD := unfold(D,-length C -1 , -1);
    differentialModule ((C**compD).dd_0)
    )

tensorDM(ChainComplexMap,DifferentialModule) := (phi,D) -> (phin = mapDM(phi)**id_(D_0);
    map(tensorDM(target phi,D),tensorDM(source phi,D),i -> phin)
    )


--cone of a morphism of DM's (viewed as morphism of complexes)
--Input: map of differential modules
--Output: the cone differential module.
coneDM = method();
coneDM ChainComplexMap := phi -> differentialModule matrix{{(target phi).dd_1,phi_1},{0,-(source phi).dd_1}};
...
doc ///
   Key 
    mapDM
    (mapDM,ChainComplexMap)
   Headline
    Given a morphism of complexes, there is an induced morphism of differential modules.
   Usage
    mapDM(phi)
   Inputs
    phi: a morphism of complexes
   Outputs
    : the induced morphism of differential modules
   Description
    Text
      Given a morphism of complexes, there is an induced morphism of differential modules.
    Example
...
doc ///
   Key 
   extendDM
   (extendDM,ChainComplex,ChainComplex,ModuleMap)
   Headline
    Compute the induced map of differential modules arising from 
    an induced comparison map of complexes.
   Usage
    extendDM(F,G,phi)
   Inputs
   F: a chain complex
   G: a chain complex
    phi: a map between F and G in homological degree 0.
   Outputs
    : the induced morphism of differential modules
   Description
    Text
...
doc ///
   Key 
   tensorDM
   (tensorDM,ChainComplex,DifferentialModule)
   Headline
    Compute the tensor product of a chain complex with a differential module.
   Usage
    tensorDM(C,D)
   Inputs
   C: a chain complex
   D: a differential module
   Outputs
    : the tensor product of C and D
   Description
    Text
      Compute the tensor product of a chain complex with a differential module.
    Example
...
doc ///
   Key 
   coneDM
   (coneDM,ChainComplexMap)
   Headline
    Compute the cone of a morphism of differential modules.
   Usage
    coneDM(phi)
   Inputs
   phi: a morphism of differential modules
   Outputs
    : the cone of phi
   Description
    Text
